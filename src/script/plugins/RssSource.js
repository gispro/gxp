...
var	rssStore = new Ext.data.JsonStore({ 
	...
	listeners :
	{
		load : function()
		{
//			alert("rssStore.listeners - load JSON : " + this.data.length);
			dataLoaded    = true;
			downloadStart = false;
//			console.log ('rssStore.listeners - load JSON : ' + this.data.length);
		},
		loadexception : function(o, arg, nul, e)
		{
			alert("rssLoader.listeners - LoadException : " + e);
		} 
	}  
});
...
/*	
	layersStore : new Ext.data.JsonStore({ 
		...
		listeners :
		{
			load : function()
			{
				alert("rssLoader.listeners - load JSON : " + this.data.length);
...
*/	
	...
	{
/*	
//		console.log ('gxp.plugins.RssSource - getLayersStore : dataLoaded = ' + dataLoaded);
		if (!dataLoaded && !downloadStart)
		{
//			this.layersStore.load();
			rssStore.load();
...
//		console.log ('gxp.plugins.RssSource - createLayerRecord : dataLoaded = ' + dataLoaded);
/*
		if (!dataLoaded && !downloadStart)
		{
//			this.layersStore.load();
...
*/
	...
	{
//		console.log ('createRecord : title = ' + title + ', name = ' + name + ', ' + this.layersStore + ', ' + this.layersStore.data + ', ' + this.layersStore.data.length);
		var record = new GeoExt.data.LayerRecord();
		var layer  = null;
		var name;
		var icon;
		...
		{
//			console.log ('createRecord : idx = ' + idx + ', ' +  this.layersStore.data.items[idx] + ', ' + this.layersStore.data.items[idx].get('title'));
...
//				var parts = this.layersStore.layers[idx].url.split("/");
